    ...
    switch (message->what)
    {
            ...
            if (m_badArchive == true)
            {
                ...
                if (msg->FindString(kErrorString, &errString) == B_OK)
                {
                    ...
                    BAlert* errAlert = new BAlert("Error", alertStr, B_TRANSLATE("View report"),
                                                  B_TRANSLATE(skCancelString), NULL, B_WIDTH_AS_USUAL,
        ...
        case M_COUNT_COMPLETE:
        {
                    ...
                    _prefs_add.FindBoolDef(kPfConfirmDropAdd, true))
            {
                BAlert* confirmAlert = new BAlert("confirm", B_TRANSLATE("Continue adding the dropped files to the archive?"),
                                                  B_TRANSLATE("Add files"), B_TRANSLATE(skCancelString),
        ...
        case M_ADD_DONE:
        {
            ...
            if (result == BZR_CANCEL_ARCHIVER)
            {
                ...
                BAlert* errAlert = new BAlert("Error", alertStr, B_TRANSLATE(skCloseWindowString),
        ...
        case M_SEARCH:
        {
            ...
            if (foundCount == -1)
            {
                ...
                errorStr << "\n\n'" << errorString << "'";
                (new BAlert("", errorStr.String(), B_TRANSLATE(skOKString), NULL, NULL,
        ...
        case M_SAVE_AS_DEFAULT:
        {
            BAlert* alert = new BAlert(B_TRANSLATE_SYSTEM_NAME(K_APP_TITLE),
                                       ...
                                       B_TRANSLATE(skCancelString), B_TRANSLATE(skSaveString),
                                       NULL, B_WIDTH_AS_USUAL, B_EVEN_SPACING, B_INFO_ALERT);
        ...
        case M_SAVE_ARK_AS_DEFAULT:
        {
            ...
            BAlert* alert = new BAlert(B_TRANSLATE_SYSTEM_NAME(K_APP_TITLE), confirmStr, B_TRANSLATE(skCancelString),
                                       B_TRANSLATE(skSaveString), NULL, B_WIDTH_AS_USUAL, B_EVEN_SPACING, B_INFO_ALERT);
            alert->SetDefaultButton(alert->ButtonAt(1));
            alert->SetShortcut(0, B_ESCAPE);
            if (alert->Go() == 1)
        ...
        case M_FILE_DELETE:
        {
            ...
            BAlert* confirm = new BAlert("", alertStr, B_TRANSLATE(skCancelString),
                                         B_TRANSLATE(skDeleteString), B_TRANSLATE("Move to Trash"),
...
void MainWindow::AddToolBar()
{
    ...
    m_newButton = new ImageButton(buttonRect, "MainWindow:New", B_TRANSLATE(skNewString), _bmps->m_tbarNewBmp,
    ...
    m_openButton = new ImageButton(buttonRect, "MainWindow:Open", B_TRANSLATE(skOpenString), _bmps->m_tbarOpenBmp,
                                   NULL, new BMessage(M_FILE_OPEN), true, backColor, kBelowIcon);
    m_openButton->SetToolTip(const_cast<char*>(B_TRANSLATE(skOpenExistingString)));

    m_searchButton = new ImageButton(buttonRect, "MainWindow:Search", B_TRANSLATE(skSearchString),
    ...
    m_viewButton = new ImageButton(buttonRect, "MainWindow:View", B_TRANSLATE(skViewString),
    ...
    m_addButton = new ImageButton(buttonRect, "MainWindow:Add", B_TRANSLATE(skAddString),
    ...
    m_deleteButton = new ImageButton(buttonRect, "MainWindow:Delete", B_TRANSLATE(skDeleteString),
...
void MainWindow::AddListView()
{
    ...
    m_fileNameColumn = new CLVColumn(B_TRANSLATE(skNameString), 300, columnFlags | CLV_LOCK_AT_BEGINNING | CLV_NOT_MOVABLE);
    m_sizeColumn = new CLVColumn(B_TRANSLATE(skSizeString), 96, columnFlags | CLV_RIGHT_JUSTIFIED);
    m_packedColumn = new CLVColumn(B_TRANSLATE(skPackedString), 72, columnFlags | CLV_RIGHT_JUSTIFIED);
    m_ratioColumn = new CLVColumn(B_TRANSLATE(skRatioString), 64, columnFlags | CLV_RIGHT_JUSTIFIED);
    m_pathColumn = new CLVColumn(B_TRANSLATE(skPathString), 128, columnFlags);
    m_dateColumn = new CLVColumn(B_TRANSLATE(skDateString), 142, columnFlags);
    m_methodColumn = new CLVColumn(B_TRANSLATE(skMethodString), 72, columnFlags);
    m_crcColumn = new CLVColumn(B_TRANSLATE(skCRCString), 64, columnFlags);
...
void MainWindow::UpdateListItemNeeders(bool enable)
{
    ...
    m_mainMenu->m_editMenu->FindItem(B_TRANSLATE(skSelectAllString))->SetEnabled(enable);
...
void MainWindow::DeleteFilesFromArchive()
{
    ...
    alertStr << "\n" << B_TRANSLATE("This operation cannot be reverted.");
    BAlert* warnAlert = new BAlert("Warning", alertStr, B_TRANSLATE("Delete"), B_TRANSLATE(skCancelString),
...
void MainWindow::DeleteDone(BMessage* message)
{
    ...
    switch (result)
    {
        case BZR_CANCEL_ARCHIVER:
        {
            ...
            BString alertStr(B_TRANSLATE("Cannot continue."));
            alertStr << "\n" << B_TRANSLATE("A critical operation has been cancelled and the archive is in an unknown state.");
            BAlert* errAlert = new BAlert("Error", alertStr, B_TRANSLATE(skCloseWindowString), NULL,
        ...
        case BZR_DONE: case BZR_ERRSTREAM_FOUND:
        {
            ...
            if (message->FindString(kText, &testResult) == B_OK)
            {
                ...
                if (result == BZR_ERRSTREAM_FOUND)
                {
                    ...
                    alertStr << "\n" << B_TRANSLATE("Do you want to view a detailed report?");
                    errAlert = new BAlert("Error", alertStr, B_TRANSLATE("View report"), B_TRANSLATE(skCancelString),
                                          NULL, B_WIDTH_AS_USUAL, B_EVEN_SPACING, B_STOP_ALERT);
                }
                else
                {
                    ...
                    alertStr << "\n" << B_TRANSLATE("Do you want to view a detailed report?");
                    errAlert = new BAlert("Result", alertStr, B_TRANSLATE("View report"), B_TRANSLATE(skOKString),
                                          NULL, B_WIDTH_AS_USUAL, B_EVEN_SPACING, B_INFO_ALERT);
                }
...
void MainWindow::SetupExtractPanel(BMessage* extractMessage)
{
    ...
    if (m_extractToPanel == NULL)
    {
        ...
        m_extractToPanel->SetButtonLabel(B_DEFAULT_BUTTON, B_TRANSLATE(skExtractString));
...
void MainWindow::SetupArchiver(entry_ref* ref, char* mimeString)
{
    ...
    if (m_archiver == NULL)        // Handle unsupported types
    {
        ...
        BAlert* errAlert = new BAlert("Error", errStr, B_TRANSLATE(skOKString), NULL, NULL,
    ...
    if ((errCode = m_archiver->InitCheck()) != BZR_DONE)    // Type is supported,
    {
        ...
        switch (errCode)
        {
            case BZR_BINARY_MISSING:           // Add-on couldn't trace its binary in the Binaries folder
            {
                ...
                (new BAlert("Error", alertStr, B_TRANSLATE(skOKString), NULL, NULL, B_WIDTH_AS_USUAL,
...
void MainWindow::AddArchiverMenu()
{
    if (m_createMode == false)
    {
        ...
        m_archiver->SettingsMenu()->AddItem(new BMenuItem(B_TRANSLATE(skSaveAsDefaultsString), new BMessage(M_SAVE_ARK_AS_DEFAULT)));
        m_archiver->SettingsMenu()->AddItem(new BMenuItem(B_TRANSLATE(skSaveToArchiveString), new BMessage(M_SAVE_ARK_TO_ARCHIVE)));
        ...
        if (refsMessage->FindRef("refs", i, &ref) == B_OK)
        {
            ...
            if (hashEntry)
            {
                ...
                else if (existingEntrySuper && addingEntrySuper)    // Replacement of folder
                {
                    ...
                    BAlert* confAlert = new BAlert("Confirm", confirmBufStr.String(), B_TRANSLATE(skCancelString),
                ...
                if (showError == true)
                {
                    ...
                    BAlert* errAlert = new BAlert("Error", confirmBufStr.String(), B_TRANSLATE(skOKString),
                                                  NULL, NULL, B_WIDTH_AS_USUAL, B_EVEN_SPACING, B_STOP_ALERT);
                    errAlert->SetFeel(B_MODAL_SUBSET_WINDOW_FEEL);
                    errAlert->AddToSubset(this);
                    errAlert->Go();
...
void MainWindow::SetupAddPanel()
{
    if (m_addPanel == NULL)
    {
        m_addPanel = new BFilePanel(B_OPEN_PANEL, new BMessenger(this), 0,
                                    ...
                                    new BMessage(M_ADD));
        m_addPanel->SetButtonLabel(B_DEFAULT_BUTTON, B_TRANSLATE(skAddString));
        ...
        if (m_archiver && m_archiver->SupportsPassword() && m_addPanel->Window()->LockLooper())
        {
            ...
            BTextControl* pwdText = new BTextControl(BRect(10, button->Frame().top + 2, 10 + 150, 0),
                                                     "pwdText", B_TRANSLATE(skPasswordString),
...
void MainWindow::ShowOpNotSupported() const
{
    ...
    BAlert* errAlert = new BAlert("Cannot operate", B_TRANSLATE("The archiver does not support this operation."),
                                  B_TRANSLATE(skOKString), NULL, NULL, B_WIDTH_AS_USUAL, B_EVEN_SPACING, B_STOP_ALERT);
    errAlert->SetFeel(B_MODAL_SUBSET_WINDOW_FEEL);
    errAlert->AddToSubset((BWindow*)this);
    errAlert->Go();
}
